...
class TestReceiptService(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.receipt_service = ReceiptService(self.receipt_repository, self.product_repository, self.campaign_repository)
    ...
    def test_add_product_to_receipt(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.receipt_service.add_product_to_receipt(receipt_id="2", product_id="1", quantity=2)

        # Test adding a non-existent product to the receipt
        with self.assertRaises(ValueError):
            self.receipt_service.add_product_to_receipt(receipt_id="1", product_id="2", quantity=2)
