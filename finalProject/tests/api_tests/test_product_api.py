...
def test_create_product():
    ...
    product_data = {
        "name": f"laptop",  # Ensure unique name
...
def test_read_product_endpoint_existing():
    ...
    assert response.status_code == 200  # Assert that status code is 200 OK for existing product
    response_data = response.json()
    assert response_data["product"]["name"] == "Laptop"
...
def test_list_products_endpoint():
    ...
    assert response.status_code == 200  # Assert that status code is 200 OK for list endpoint
    response_data = response.json()
